const mobileMode=window.matchMedia('(max-width: 820px)').matches;

if(mobileMode){
  document.documentElement.classList.add('m-loading');
  const guard=document.createElement('style');
  guard.textContent='@media(max-width:820px){html.m-loading body{visibility:hidden}}';
  document.head.appendChild(guard);

  const css=document.createElement('link');
  css.rel='stylesheet';
  css.href='/mobile-v4.css';
  document.head.appendChild(css);

  const runtime=document.createElement('script');
  runtime.src='/mobile-v4.js';
  runtime.onload=()=>document.documentElement.classList.remove('m-loading');
  runtime.onerror=()=>document.documentElement.classList.remove('m-loading');
