...
import {pages} from '../src/site-v8.mjs';
...
for(const [rel,html] of Object.entries(pages)){
  ...
  for(const m of html.matchAll(/src="(\/[^\"]+)"/g)){
    const src=m[1];if(!src.startsWith('/images/')&&!src.startsWith('/app-')&&!src.startsWith('/styles-')&&!src.startsWith('/quality-')&&!src.startsWith('/local-growth-')&&!src.startsWith('/flagship-'))continue;
  ...
  if(!html.includes('quality-v5.css')||!html.includes('quality-v6.css')||!html.includes('local-growth-v7.css')||!html.includes('flagship-v8.css')){console.error(`${rel}: visual quality layers missing`);failed=true}
...
for(const asset of ['styles-v3.css','app-v3.js','quality-v5.css','quality-v5.js','quality-v6.css','quality-v6.js','local-growth-v7.css','local-growth-v7.js','local-growth-v7-mobile.css','flagship-v8.css','flagship-v8.js']){
...
const flagshipCss=await fs.readFile(path.join(root,'flagship-v8.css'),'utf8').catch(()=>"");
for(const signal of ['.v8-mobile-dock','.qv5-result','.v7-lead-form','.v8-decision-ribbon','prefers-reduced-motion']){
  if(!flagshipCss.includes(signal)){console.error(`flagship v8 style missing: ${signal}`);failed=true}
}
...
for(const signal of ['ENCUENTRA TU CAMINO','COMPARADOR DE FORMATOS','ANTES DEL EXAMEN','QUÉ SIGNIFICA “APRENDER USANDO”','MATRÍCULA / QUÉ PREPARAR','ANTES DE ESCRIBIR','MÉTODO / EN MOVIMIENTO','MATRÍCULA / SIN FRICCIÓN','CONTACTO / ELIGE EL CANAL','ACADEMIA DE INGLÉS EN ÉCIJA','QUIERO INFORMACIÓN','SOLICITUD DE MATRÍCULA','¿PARA QUÉ ESCRIBES?','CONSULTA ENVIADA','COSMO CERCA DE TI / ÉCIJA','QUIERO QUE ME ORIENTÉIS','EMPIEZA POR LO IMPORTANTE','v8-mobile-dock']){
...
console.log(`QA passed: ${Object.keys(pages).length} routes, flagship v8 polish, separate mobile lead journey, official imagery, local discovery, four functional Netlify forms, thank-you flow and structured data.`);
